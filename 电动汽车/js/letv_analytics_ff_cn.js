...
function letvAlysGenSessionId(){
    var tj_sg = letvAlysGetCookie("tj_sg");
    var tj_sid = letvAlysGetCookie("tj_sid");
    if(tj_sg==null || tj_sg==""){
        tj_sid = letvAlysGenString();
        letvAlysSetCookie("tj_sg", "1", {});
        letvAlysSetCookie("tj_sid", tj_sid, {exp: 0.5});
    }else{
...
function letvAlysInit(product, platform, country, userid, ext, property){
    var tj2_lc = letvAlysGenCookie();
    var sourceUrl = letvAlysGenSourceUrl();
    var  sid = letvAlysGenSessionId();
    var tj_lc = letvAlysGetCookie("tj_lc");
    var uid = userid;
...
function letvAlyStart(product, platform, country, userid, ext, property){
        try{
            var rootDomain = getRootDomain();
        ...
        letvAlysInit(product, platform, country, userid, ext, property);
}
